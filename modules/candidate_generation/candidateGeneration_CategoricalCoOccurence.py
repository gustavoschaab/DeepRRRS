...
class CandidateGenerationCategoricalCoOccurence(CandidateGeneration):
    ...
    def defineModel(self):
        ...
        userInput = Input(shape=[600], name="user")
        userInput2 = Dense(300, activation='relu')(userInput) 
        ...
        input_vecs = Concatenate()([userInput2, coOccurenceInput2, categoricalInput2])        
        input_vecs = Dropout(0.2)(input_vecs)
        x = Dense(900, activation='relu')(input_vecs)
        x = Dropout(0.2)(x)
        ...
        model = Model(inputs=[userInput, coOccurenceInput, categoricalInput], outputs=y)
    ...
    def generateXY(self, dataset):
        ...
        return numpy.asarray(UserVector), numpy.asarray(CoOccurenceVector), numpy.asarray(CategoricalVector),numpy.asarray(Y), numpy.asarray(UserId), numpy.asarray(ItemId)

    def generateDataframe(self, UserId, ItemId, UserVector, CoOccurenceVector, CategoricalVector, ActualRating):
        self.logger.info("Start Generate Dataframe")
        columns = ['UserId', 'ItemId', 'UserVector', 'CoOccurenceVector', 'CategoricalVector', 'Actual']
        data = []
        for item in range(len(UserId)):
            ...
            data.append([user, itemId, userVector, coOccurenceVector, categoricalVector, actual])
    ...
    def generateDatasetWithPredictedValues(self, testData, model):
        ...
        for _, row in testData.iterrows():
            y = model.predict([[row['UserVector']],[row['CoOccurenceVector']],[row['CategoricalVector']]])
