...
class CandidateGenerationCategorical(CandidateGeneration):
    ...
    def defineModel(self):
        ...
        categoricalInput = Input(shape=[300], name="textual")
        categoricalInput2 = Dense(150, activation='relu')(categoricalInput) 

        input_vecs = Concatenate()([userInput2, categoricalInput2])        
        ...
        model = Model(inputs=[userInput, categoricalInput], outputs=y)
    ...
    def runNeuralNetwork(self):
        ...
        for idx, (train, test) in enumerate(kfold.split(X1, Y)):
            ...
            testData = self.generateDataframe(UserId[test], ItemId[test], X1[test], X2[test], Y[test])
    ...
    def generateXY(self, dataset):
        ...
        for _, row in dataset.iterrows():
            ...
            ItemVector.append(self.categoricalEmbeddings[row['ProductId']])
            Y.append(row['Rating'])
