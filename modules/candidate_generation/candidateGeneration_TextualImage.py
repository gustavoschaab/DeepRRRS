...
class CandidateGenerationTextualImage(CandidateGeneration):
    ...
    def defineModel(self):
        ...
        input_vecs = Concatenate()([userInput2, imageInput2, textualInput2])        
        ...
        model = Model(inputs=[userInput, imageInput, textualInput], outputs=y)
    ...
    def generateXY(self, dataset):
        ...
        return numpy.asarray(UserVector), numpy.asarray(ImageVector), numpy.asarray(TextualVector),numpy.asarray(Y), numpy.asarray(UserId), numpy.asarray(ItemId)

    def generateDataframe(self, UserId, ItemId, UserVector, ImageVector, TextualVector, ActualRating):
        self.logger.info("Start Generate Dataframe")
        columns = ['UserId', 'ItemId', 'UserVector', 'TextualVector', 'ImageVector', 'Actual']
        data = []
        for item in range(len(UserId)):
            ...
            data.append([user, itemId, userVector, textualVector, imageVector, actual])
    ...
    def generateDatasetWithPredictedValues(self, testData, model):
        ...
        for _, row in testData.iterrows():
            y = model.predict([[row['UserVector']],[row['ImageVector']],[row['TextualVector']]])
