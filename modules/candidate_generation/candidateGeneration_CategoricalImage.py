...
class CandidateGenerationCategoricalImage(CandidateGeneration):
    ...
    def defineModel(self):
        ...
        input_vecs = Concatenate()([userInput2, categoricalInput2, imageInput2])        
        ...
        model = Model(inputs=[userInput, categoricalInput, imageInput], outputs=y)
    ...
    def generateXY(self, dataset):
        ...
        return numpy.asarray(UserVector), numpy.asarray(CategoricalVector), numpy.asarray(ImageVector),numpy.asarray(Y), numpy.asarray(UserId), numpy.asarray(ItemId)

    def generateDataframe(self, UserId, ItemId, UserVector, CategoricalVector, ImageVector, ActualRating):
        self.logger.info("Start Generate Dataframe")
        columns = ['UserId', 'ItemId', 'UserVector', 'CategoricalVector', 'ImageVector', 'Actual']
        data = []
        for item in range(len(UserId)):
            ...
            data.append([user, itemId, userVector, categoricalVector, imageVector, actual])
    ...
    def generateDatasetWithPredictedValues(self, testData, model):
        ...
        for _, row in testData.iterrows():
            y = model.predict([[row['UserVector']],[row['CategoricalVector']],[row['ImageVector']]])
