...
class CandidateGenerationCategoricalCoOccurenceImage(CandidateGeneration):
    ...
    def defineModel(self):
        ...
        userInput = Input(shape=[4696], name="user")
        ...
        input_vecs = Concatenate()([userInput2, categoricalInput2, coOccurenceInput2, imageInput2])        
        input_vecs = Dropout(0.2)(input_vecs)
        x = Dense(600, activation='relu')(input_vecs)
        x = Dropout(0.2)(x)
        ...
        model = Model(inputs=[userInput, categoricalInput, coOccurenceInput, imageInput], outputs=y)
    ...
    def generateXY(self, dataset):
        ...
        return numpy.asarray(UserVector), numpy.asarray(CategoricalVector), numpy.asarray(CoOccurenceVector), numpy.asarray(ImageVector),numpy.asarray(Y), numpy.asarray(UserId), numpy.asarray(ItemId)

    def generateDataframe(self, UserId, ItemId, UserVector, CategoricalVector, CoOccurenceVector, ImageVector, ActualRating):
        self.logger.info("Start Generate Dataframe")
        columns = ['UserId', 'ItemId', 'UserVector', 'CategoricalVector', 'CoOccurenceVector', 'ImageVector', 'Actual']
        data = []
        for item in range(len(UserId)):
            ...
            data.append([user, itemId, userVector, categoricalVector, coOccurenceVector, imageVector, actual])
    ...
    def generateDatasetWithPredictedValues(self, testData, model):
        ...
        for _, row in testData.iterrows():
            y = model.predict([[row['UserVector']],[row['CategoricalVector']],[row['CoOccurenceVector']],[row['ImageVector']]])
