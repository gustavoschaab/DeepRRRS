...
class CandidateGenerationTextualCategorical(CandidateGeneration):
    ...
    def defineModel(self):
        ...
        input_vecs = Concatenate()([userInput2, textualInput2, categoricalInput2])        
        ...
        model = Model(inputs=[userInput, textualInput, categoricalInput], outputs=y)
    ...
    def generateXY(self, dataset):
        ...
        return numpy.asarray(UserVector), numpy.asarray(TextualVector), numpy.asarray(CategoricalVector),numpy.asarray(Y), numpy.asarray(UserId), numpy.asarray(ItemId)

    def generateDataframe(self, UserId, ItemId, UserVector, TextualVector, CategoricalVector, ActualRating):
        self.logger.info("Start Generate Dataframe")
        columns = ['UserId', 'ItemId', 'UserVector', 'TextualVector', 'CategoricalVector', 'Actual']
        data = []
        for item in range(len(UserId)):
            ...
            data.append([user, itemId, userVector, textualVector, categoricalVector, actual])
    ...
    def generateDatasetWithPredictedValues(self, testData, model):
        ...
        for _, row in testData.iterrows():
            y = model.predict([[row['UserVector']],[row['TextualVector']],[row['CategoricalVector']]])
