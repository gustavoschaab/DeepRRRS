...
class CandidateGenerationTextualCategoricalCoOccurence(CandidateGeneration):
    ...
    def defineModel(self):
        ...
        userInput = Input(shape=[900], name="user")
        userInput2 = Dense(450, activation='relu')(userInput) 
        ...
        input_vecs = Concatenate()([userInput2, textualInput2, categoricalInput2, coOccurenceInput2])        
        input_vecs = Dropout(0.2)(input_vecs)
        x = Dense(1350, activation='relu')(input_vecs)
        x = Dropout(0.2)(x)
        x = Dense(675, activation='relu')(x)
        x = Dropout(0.2)(x)
        x = Dense(900, activation='relu')(x)
        x = Dropout(0.5)(x)
        y = Dense(len(self.lb.classes_), activation="softmax")(x)
        model = Model(inputs=[userInput, textualInput, categoricalInput, coOccurenceInput], outputs=y)
    ...
    def runNeuralNetwork(self):
        ...
        X1, X2, X3, X4, Y, UserId, ItemId = self.generateXY(ratingsDataset) 
        ...
        for idx, (train, test) in enumerate(kfold.split(X1, Y)):
            ...
            H = model.fit([X1[train], X2[train], X3[train], X4[train]] , Y_train, validation_data=([X1[test], X2[test], X3[test], X4[test]], Y_test), epochs=self.epochs, batch_size=32, callbacks=[early_stopping], verbose=2)
            
            scores = model.evaluate([X1[test], X2[test], X3[test], X4[test]], Y_test, verbose=0)
            ...
            testData = self.generateDataframe(UserId[test], ItemId[test], X1[test], X2[test], X3[test], X4[test], Y[test])
    ...
    def generateXY(self, dataset):
        ...
        return numpy.asarray(UserVector), numpy.asarray(TextualVector), numpy.asarray(CategoricalVector), numpy.asarray(CoOccurenceVector), numpy.asarray(Y), numpy.asarray(UserId), numpy.asarray(ItemId)

    def generateDataframe(self, UserId, ItemId, UserVector, TextualVector, CategoricalVector, CoOccurenceVector, ActualRating):
        self.logger.info("Start Generate Dataframe")
        columns = ['UserId', 'ItemId', 'UserVector', 'TextualVector', 'CategoricalVector', 'CoOccurenceVector', 'Actual']
        data = []
        for item in range(len(UserId)):
            ...
            data.append([user, itemId, userVector, textualVector, categoricalVector, coOccurenceVector, actual])
    ...
    def generateDatasetWithPredictedValues(self, testData, model):
        ...
        for _, row in testData.iterrows():
            y = model.predict([[row['UserVector']],[row['TextualVector']],[row['CategoricalVector']],[row['CoOccurenceVector']]])
